...
static NSNumber* FeatureSwitchOverrideValueForKey(NSString* key) {
    // Unlock Twitter's native premium color themes so the accent color the user
    // picks is applied everywhere (links, buttons, highlights) by Twitter itself.
    // This is how the custom accent propagates natively — same as Moe.
    if ([key isEqualToString:@"blue_premium_themes_enabled"] ||
        [key isEqualToString:@"blue_custom_navigation_bar_enabled"] ||
        [key isEqualToString:@"blue_custom_app_icon_enabled"] ||
        [key isEqualToString:@"app_customization_custom_primary_color_enabled"] ||
        [key isEqualToString:@"subscriptions_is_subscribed"]) {
        return @YES;
    } 
